...
module gotcha (
    ...
    wire        nc_J4_pin6, nc_J4_pin8;
    wire        nc_H4_g3;
    wire        nc_E4_g1, nc_E4_g2;
    // (no remaining D6 stubs)
    wire        nc_K4_g2, nc_K4_g3;
    wire        nc_K5_g4;
    ...
    //   Gate 3: still unused (Phase 4 /* Right Control */ wires it).
    // ==================================================================
    ttl_7410 u_H4 (
    ...
    //   Gates 2 and 3 stubbed (used in Phase 3+ player control logic).
    // ==================================================================
    ttl_7408 u_J4 (
    ...
    //     Drives both C4 and D4 /PE (sync load when both counters hit 15).
    //   Gate 4 (Phase 1 playfield): ~(B4.Q2 & V2) = E4_g4.
    //   Gates 1, 2 still unused (Phase 4 /* Right Control */).
    ttl_7400 u_E4 (
    ...
    // B1 - 7420 dual 4-input NAND.  Gate 2 only used in Phase 2; selects
    //      between two BCD sources at I7 based on screen position.
    //   pin9=F4./Q2, pin10=~H64, pin12=L5_g4_out, pin13=V32 -> pin8.
    //   Gate 1 (pins 1,2,4,5,6) reserved for Phase 4 /* Coin */.
    ttl_7420 u_B1 (
        .pin1  (GND), .pin2  (GND), .pin4 (GND), .pin5 (GND), .pin6 (),
    ...
    // Picture — Phase 2 canonical video output.  F6.pin13 = ~(HBLANK | D6_g1),
    // i.e. the F8 NAND combiner (maze + 7-seg score) gated by HBLANK and the
    // CATCHOS_n catch-overlay.  Player sprites (VIDEO[2], VIDEO[3] from F1)
    // are added in Phase 3.
    assign video = F6_pin13 ? 8'hFF : 8'h00;
