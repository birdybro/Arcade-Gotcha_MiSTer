...
- **Reset convention:** every stateful primitive (`ttl_7474/74107/7490/7493/9316/9602/555_mono/latch`) and `gotcha_sound` has a synchronous `rst` input (2nd port, right after `clk_sys`) that forces its state back to the power-on value; `ttl_latch` re-runs its ~1µs power-on pulse so the ATTRACT/game-state chain re-initialises. `emu`'s `reset` (= `RESET | status[0] | buttons[1]`) drives `gotcha.reset`, which fans out to every instance's `.rst(reset)`. **When adding a new clocked chip instance, wire `.rst(reset)`** (purely combinational primitives — 7400/02/04/08/10/20/27/30/48/86/74153/74157 — have no `rst`). This is what makes OSD-Reset actually work despite the real PCB having no reset line.
